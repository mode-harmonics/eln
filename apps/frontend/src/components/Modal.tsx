...
export function Modal({ open, onClose, title, children, footer, maxWidth = "lg" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/40 backdrop-blur-sm transition-opacity">
      <div
        className={cn(
          "bg-white rounded-md shadow border border-gray-100 w-full flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-200 ease-out m-4",
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 shrink-0">
          ...
          <button onClick={onClose} className="p-1 rounded text-gray-400 hover:text-gray-900 bg-transparent transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
